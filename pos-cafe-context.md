...
> **Cập nhật 2026-06-12 sau integration/cloud checklist.** Tóm tắt nhanh cho mỗi session.
...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity và architecture boundary cleanup đã implement/push trên code branch `codex/stream-db-rpc` tới commit `9f32291`. Integration checklist và Supabase cloud setup checklist đã có trong docs. `codex/code-foundation` giữ cố định cho UI branch, chưa merge logic vào. Bước kế tiếp: chờ UI branch xong rồi tạo branch integration riêng từ UI branch và merge `codex/stream-db-rpc` theo checklist; nếu user cấp Supabase project/env trước thì chuẩn bị cloud apply/smoke theo cloud setup checklist.
