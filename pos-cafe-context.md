...
- **Chưa push/commit các slice local mới theo yêu cầu:** code worktree `D:\Workspace\pos-cafe-code` đang ở `codex/ui-logic-integration`, docs worktree `D:\Workspace\pos-cafe-docs` đang ở `docs`. Các commit đã push gần nhất trên integration branch vẫn là merge/UI foundation + Employees slice; Menu/Floor editor và Report/History polish đang local.
...
- **Đã xong:** DB/RPC foundation, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks nền, Supabase cloud RPC/REST E2E, UI foundation binding, Employees binding slice, Menu editor changeset save binding, Floor editor changeset save binding, và Report/History polish binding.
- **Validation mới nhất:** `npm run test -- reportHistoryDrawer`, `npm run test` (20 files/75 tests), `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped; lần đầu thiếu npm shim trong PATH rồi rerun pass), `git diff --check`, boundary grep UI/Supabase.
- **Phase nhỏ tiếp theo:** chạy **Supabase-mode UI E2E** với Store Key/test account rõ ràng.
...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity, architecture boundary cleanup và Supabase cloud pgcrypto fix đã implement/push trên code branch `codex/stream-db-rpc` tới commit `ae1523b`. Supabase cloud E2E đã pass ở mức RPC/REST trực tiếp. UI branch `codex/code-foundation` đã có UI mock + spacing fix `6009b47`. Branch `codex/ui-logic-integration` đã tạo từ UI checkpoint, merge `codex/stream-db-rpc`, push merge commit `1c420d9`; validation pass. Binding nền UI thật đã push commit `47f17f6`: session bootstrap/pair/create/passcode dùng session hooks, realtime invalidation bật theo store session, POS floor/takeaway/order/payment/settings/clear-demo load/mutate qua feature hooks/AppPorts, mock session mặc định unpaired để đi đúng landing flow. Employees binding slice đã push commit `4d7d1ae`: thêm `employee.listEmployees()` cho admin list, giữ `listActiveEmployees()` cho passcode, Employees drawer tạo/sửa/tạm khoá/reset PIN qua admin hooks/AppPorts. Menu editor changeset save đã implement local trên `codex/ui-logic-integration`: UI tạo diff `created/updated/deleted`, sinh UUID client cho draft mới, gọi `menu.saveMenuChanges`, refetch menu sau save, và có component tests cho create/tombstone. Floor editor changeset save cũng đã implement local: UI tạo diff `areas/tables/decorItems`, lưu logical stage `1600x900` theo percent scale-to-fit, không gửi/ghi đè `tables.status`, gọi `floorPlan.saveFloorPlan`, refetch sau save, và có component tests cho create/update/tombstone/scale. Report/History polish đã implement local: Order History dùng `useOrderHistoryQuery` + `useOrderDetailQuery`, custom date range + server pagination; Report dùng daily `getCoreReport` qua `useCoreReportsQuery` để aggregate 7 ngày/tháng/custom và dùng history query cho recent paid orders; mock report repo lọc đúng `businessDate`. Validation mới pass: `npm run test -- reportHistoryDrawer`, `npm run test` (20 files/75 tests), `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped), `git diff --check`, boundary grep. Bước kế tiếp: chạy Supabase-mode UI E2E có Store Key/test account rõ ràng; chưa merge integration branch vào `codex/code-foundation`/main.
