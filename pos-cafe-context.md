...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity, architecture boundary cleanup và Supabase cloud pgcrypto fix đã implement/push trên code branch `codex/stream-db-rpc` tới commit `ae1523b`. Supabase cloud E2E đã pass ở mức RPC/REST trực tiếp. UI branch `codex/code-foundation` đã có UI mock + spacing fix `6009b47`. Branch `codex/ui-logic-integration` đã tạo từ UI checkpoint, merge `codex/stream-db-rpc`, push merge commit `1c420d9`; validation pass: `npm run build`, `npm run test` (16 files/60 tests), `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped), `git diff --check`, conflict/import scan. Bước kế tiếp: bind UI thật theo `docs/superpowers/specs/2026-06-11-pos-cafe-ui-logic-binding.md` và chạy UI cloud E2E trên Supabase mode; chưa merge integration branch vào `codex/code-foundation`/main.
