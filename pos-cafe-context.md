...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity, architecture boundary cleanup và Supabase cloud pgcrypto fix đã implement/push trên code branch `codex/stream-db-rpc` tới commit `ae1523b`. Supabase cloud E2E đã pass ở mức RPC/REST trực tiếp. UI branch `codex/code-foundation` đã có UI mock + spacing fix `6009b47`. Branch `codex/ui-logic-integration` đã tạo từ UI checkpoint, merge `codex/stream-db-rpc`, push merge commit `1c420d9`; validation pass. Binding nền UI thật đã push commit `47f17f6`: session bootstrap/pair/create/passcode dùng session hooks, realtime invalidation bật theo store session, POS floor/takeaway/order/payment/settings/clear-demo load/mutate qua feature hooks/AppPorts, mock session mặc định unpaired để đi đúng landing flow. Validation pass: `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `npm run test` (16 files/60 tests), `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped), `git diff --check`, boundary grep. Bước kế tiếp: bind admin editor changesets thật (employees/menu/floor/report/history còn mock/local một phần) và chạy Supabase-mode UI E2E có Store Key/test account rõ ràng; chưa merge integration branch vào `codex/code-foundation`/main.
