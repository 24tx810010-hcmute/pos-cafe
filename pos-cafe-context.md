...
| Backend contract | SQL migrations trong `supabase/migrations` apply cloud trực tiếp, hiện có 4 file: `001` schema/enums, `002` indexes/RLS/triggers, `003` RPC/functions, `004` realtime publication. Ba file đầu là foundation chính. RPC critical: `get_next_store_no`(sequence), `verify_employee_pin`, `submit_order_changes`, `pay_order`, `clear_demo_data`; `void_order` reserved/admin/future. Local validation 2026-06-11: PostgreSQL 18.4 qua Scoop, temp DB, apply `001`/`002`/`003` pass, RPC smoke/negative smoke pass. Supabase cloud validation 2026-06-12 pass: env/Auth settings OK, tables/RPC reachable, cloud E2E create store test + seed tối thiểu + verify PIN + submit/pay/clear-demo pass. Migration fix `ae1523b` dùng `extensions.crypt/gen_salt` cho pgcrypto trên Supabase cloud |
