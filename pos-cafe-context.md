...
> **Cập nhật 2026-06-21 sau architecture/portal hardening.** Tóm tắt nhanh cho mỗi session.
> **App truth hiện tại:** `origin/main` commit `5bc17db` (`feat(app): add drawer overlay motion`).
...
- **Main mới nhất:** `origin/main@5bc17db` đã nhận architecture folder refactor, portal popup/drawer primitives, drawer overlay click-close và slide-in motion.
...
## Frontend/UI
Zustand giữ app/draft state, TanStack Query giữ server state/cache, RHF+Zod validate form. UI tiếng Việt, primary teal `#0F766E`, neutral modern. Landscape-first từ desktop/tablet tới phone xoay ngang; phone landscape vẫn truy cập đủ MVP, không cắt POS/report/editor theo thiết bị; phone portrait hiện hướng dẫn xoay ngang. App shell dùng left rail; module mở drawer qua `PortalDrawer` trong workspace viewport, overlay mặc định `rgba(0,0,0,0.2)`, click overlay đóng drawer và có slide-in theo placement. Popup/confirm dùng `PortalPopup`. Body/shell không scroll tự do: drawer cố định viewport, header/action bar sticky, từng pane tự `overflow:auto`. Module giữ mental model 3 vùng trên mọi màn ngang: trái category/filter/navigation, giữa content chính, phải detail/cart/payment/properties; order drawer category trái + menu grid giữa + cart phải, cart footer sticky. Floor plan dùng logical stage `1600x900`, scale-to-fit; pan/zoom canvas riêng; drag/resize mutate DOM qua ref/requestAnimationFrame, pointerup mới commit draft.
