...
> **Cập nhật 2026-06-14 sau Supabase realtime UI E2E.** Tóm tắt nhanh cho mỗi session.
...
## Trạng thái hiện tại (2026-06-14)

- **Chưa push/commit phần local mới của phase Demo Hardening:** code worktree `D:\Workspace\pos-cafe-code` đang ở `codex/ui-logic-integration`, docs worktree `D:\Workspace\pos-cafe-docs` đang ở `docs`. Supabase UI E2E + realtime migration đã push trên code commit `9ccfd48` và docs commit `f95d767`; phần mới local hiện gồm UI error toast formatter, drawer loading/error guards, clear-demo safety guard, payment/order conflict refetch, và `demoHardening.test.tsx`.
...
- **Đã xong:** DB/RPC foundation, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks nền, Supabase cloud RPC/REST E2E, UI foundation binding, Employees binding slice, Menu editor changeset save binding, Floor editor changeset save binding, Report/History polish binding, Supabase single-browser UI E2E, Supabase 2-browser realtime UI E2E sau migration `004`, và Demo Hardening slice đầu cho drawer error/guard states.
- **Validation mới nhất:** `npm run test -- demoHardening uiError` (15 tests), `npm run test` (21 files/80 tests), `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped), `npm run smoke:supabase` (1 passed/1 skipped).
- **Phase nhỏ tiếp theo:** chạy `git diff --check` + boundary grep sau docs, rồi commit/push Demo Hardening nếu user yêu cầu; tiếp tục polish nhỏ còn lại nếu cần.
...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity, architecture boundary cleanup và Supabase cloud pgcrypto fix đã implement/push trên code branch `codex/stream-db-rpc` tới commit `ae1523b`. Supabase cloud E2E đã pass ở mức RPC/REST trực tiếp. UI branch `codex/code-foundation` đã có UI mock + spacing fix `6009b47`. Branch `codex/ui-logic-integration` đã tạo từ UI checkpoint, merge `codex/stream-db-rpc`, push merge commit `1c420d9`; validation pass. Binding nền UI thật đã push commit `47f17f6`: session bootstrap/pair/create/passcode dùng session hooks, realtime invalidation bật theo store session, POS floor/takeaway/order/payment/settings/clear-demo load/mutate qua feature hooks/AppPorts, mock session mặc định unpaired để đi đúng landing flow. Employees binding slice đã push commit `4d7d1ae`: thêm `employee.listEmployees()` cho admin list, giữ `listActiveEmployees()` cho passcode, Employees drawer tạo/sửa/tạm khoá/reset PIN qua admin hooks/AppPorts. Menu/Floor editor changesets + Report/History polish đã push code commit `9b58732`: editor save dùng changesets qua ports, report/history dùng query thật và component tests. Supabase UI E2E + realtime migration đã push code commit `9ccfd48`: `npm run smoke:supabase` tạo store cloud thật, unlock admin, tạo + thanh toán order, kiểm tra history/report; thêm 2-browser realtime test cho table status invalidation và migration `004_realtime_publication.sql`. User đã apply migration lên Supabase cloud, realtime E2E pass với `RUN_SUPABASE_REALTIME_E2E=1 npm run smoke:supabase` (2 passed). Demo Hardening local mới thêm: UI error toast formatter có title, payment/order conflict refetch, loading/error states cho payment/order/floor/takeaway/settings/menu/floor editor, clear-demo guard khi open-order check loading/error, copy "mock" còn sót đổi sang demo/online, và component tests `demoHardening.test.tsx`. Validation mới pass: `npm run test -- demoHardening uiError`, `npm run test` (21 files/80 tests), `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `VITE_DATA_MODE=mock npm run smoke` (13 passed/7 skipped), `npm run smoke:supabase` (1 passed/1 skipped). Bước kế tiếp: `git diff --check` + boundary grep sau docs, rồi commit/push Demo Hardening nếu user yêu cầu; chưa merge integration branch vào `codex/code-foundation`/main.
