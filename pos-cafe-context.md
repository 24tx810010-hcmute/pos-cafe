...
> **Cập nhật 2026-06-12 sau architecture boundary cleanup.** Tóm tắt nhanh cho mỗi session.
...
| Backend contract | SQL migrations trong `supabase/migrations` apply cloud trực tiếp, 3 file foundation: schema/enums, indexes/RLS/triggers, RPC/functions. RPC critical: `get_next_store_no`(sequence), `verify_employee_pin`, `submit_order_changes`, `pay_order`, `clear_demo_data`; `void_order` reserved/admin/future. Local validation 2026-06-11: PostgreSQL 18.4 qua Scoop, temp DB, apply `001`/`002`/`003` pass, RPC smoke/negative smoke pass; Supabase cloud migration là bước setup riêng |
...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity và architecture boundary cleanup đã implement/push trên code branch `codex/stream-db-rpc` tới commit `9f32291`. `codex/code-foundation` giữ cố định cho UI branch, chưa merge logic vào. Bước kế tiếp an toàn trong lúc chờ UI: viết integration checklist + Supabase cloud setup checklist, hoặc chuẩn bị Supabase cloud apply/smoke khi có project/env. Sau khi UI xong thì tạo branch integration riêng từ UI branch và merge `codex/stream-db-rpc` vào đó để xử lý conflict/test tổng.
