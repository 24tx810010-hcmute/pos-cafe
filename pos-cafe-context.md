...
> **Cập nhật 2026-06-19 sau Virtual Printer + Receipt Layout docs.** Tóm tắt nhanh cho mỗi session.
...
- **Phase nhỏ tiếp theo:** Virtual Printer + Receipt Layout. `PrintPort` vẫn là browser HTML print-preview, không phải native printer. Boundary bắt buộc: `UI/features -> AppPorts/IPrintPort -> print adapter`; không để Supabase lọt vào UI/features/core/domain/ports. Receipt payload cần có item note, `lineTotal`, tên/địa chỉ/footer quán, employee/cashier, `printedAt`/`paidAt`, `paymentMethod`. Nếu cần đổi schema/RPC thì thêm migration `005`, không sửa migration `003` đã apply/live. Docs branch `docs` giữ riêng và không merge vào `main`.
