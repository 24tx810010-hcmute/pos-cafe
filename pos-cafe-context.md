...
> **Cập nhật 2026-07-20 sau phase 20 phân quyền per-employee.** Tóm tắt nhanh cho mỗi session.
> **App truth hiện tại:** `main` commit `de35d10`; migration 012 đã apply và verify cloud.
...
- **Main đã đối chiếu:** `main@de35d10`; phase 20 phân quyền thao tác theo từng nhân viên và migration 012 đã commit.
...
- **Đã xong trong code:** DB/RPC foundation, Supabase/mock adapters, Store/Auth/Seed, POS order/payment, admin, realtime, UI binding/hardening, instant pay, hủy đơn paid, report audit và editor quyền per-employee cho 5 action runtime.
- **Validation phase 20:** `npm test` pass 46 files/243 tests, `npm run build` pass với Vite chunk-size warning đã biết, `npm run smoke` pass 27/18 theo điều kiện viewport, `npm run smoke:supabase` pass 5/5.
