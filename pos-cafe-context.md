...
> **Cập nhật 2026-06-22 sau app shell/floor workspace cleanup.** Tóm tắt nhanh cho mỗi session.
> **App truth hiện tại:** `origin/main` commit `4bf9764` (`refactor(ui): simplify app shell`).
...
- **Main mới nhất:** `origin/main@4bf9764` đã nhận architecture folder refactor, portal popup/drawer primitives, drawer overlay click-close/slide-in motion, `LeftNav` tách khỏi shell và floor workspace bỏ top chrome riêng.
...
- **Đã xong:** DB/RPC foundation, Supabase adapter, mock adapter, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, Supabase cloud E2E, realtime invalidation, UI binding, Demo Hardening, Vercel deploy config, live payment sync hardening, architecture guard, adapter split, browser print adapter, portal overlay primitives và cleanup app shell/floor workspace.
- **Validation mới nhất trên main:** `npm test` pass 34 files/148 tests, `npm run build` pass với Vite chunk-size warning đã biết, `npm run smoke` pass 13/7. `npm run smoke:supabase` không chạy lại trong pass UI này vì không đổi backend/realtime contract.
...
| Backend contract | SQL migrations trong `supabase/migrations` apply cloud trực tiếp: `001` schema/enums, `002` indexes/RLS/triggers, `003` RPC/functions, `004` realtime publication, `005`–`006` menu image storage, `007` full wipe, `008` shared modifiers, `009` partial payment (superseded), `010` instant-pay split-order, `011` void-paid-order (thêm cột hủy + `employees.permission_overrides`, tạo lại `verify_employee_pin`, hiện thực `void_order`). RPC critical: `get_next_store_no`(sequence), `verify_employee_pin`, `submit_order_changes`, `pay_order`, `pay_order_items`, `void_order` (hủy đơn đã thanh toán), `clear_demo_data`. Local validation 2026-06-11: PostgreSQL 18.4 qua Scoop, temp DB, apply `001`/`002`/`003` pass, RPC smoke/negative smoke pass. Supabase cloud validation 2026-06-12 pass: env/Auth settings OK, tables/RPC reachable, cloud E2E create store test + seed tối thiểu + verify PIN + submit/pay/clear-demo pass. Migration fix `ae1523b` dùng `extensions.crypt/gen_salt` cho pgcrypto trên Supabase cloud |
