...
> **Cập nhật 2026-06-14 sau merge main + branch cleanup.** Tóm tắt nhanh cho mỗi session.
...
- **Đã xong:** DB/RPC foundation, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks nền, Supabase cloud RPC/REST E2E, UI foundation binding, Employees binding slice, Menu editor changeset save binding, Floor editor changeset save binding, Report/History polish binding, Supabase single/2-browser UI E2E, Demo Hardening slice đầu, Demo Readiness runbook và merge về `main`.
- **Validation mới nhất trên main:** `npm run test` (21 files/80 tests), `npm run build`, `VITE_DATA_MODE=supabase npm run build`, `git diff --check` đều pass sau merge. Trước đó Demo Readiness đã pass `RUN_SUPABASE_REALTIME_E2E=1 npm run smoke:supabase` (2 passed).
- **Phase nhỏ tiếp theo:** deploy verification trên Vercel/Supabase env thật, smoke live URL, rồi diễn tập demo 2 thiết bị.
...
## Bước kế
DB/RPC, Supabase adapter foundation, Store/Auth/Seed logic, POS order/payment hooks, admin hooks, integration support utilities, adapter hardening tests, mock data parity, architecture boundary cleanup, Supabase cloud pgcrypto fix, UI binding, editor changesets, report/history polish, Supabase UI E2E, realtime migration, Demo Hardening và Demo Readiness đã hoàn tất. Code + docs đã được merge về `main` và push. Bước kế tiếp: deploy `main` lên Vercel với Supabase env thật, chạy smoke live URL, rồi diễn tập demo 2 thiết bị theo runbook.
