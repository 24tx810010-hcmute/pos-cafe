...
| Backend contract | SQL migrations trong `supabase/migrations`; RPC critical: `get_next_store_no`(sequence), `verify_employee_pin`, `submit_order_changes`, `pay_order`, `clear_demo_data`; `void_order` reserved/admin/future |
...
| **Demo** | Seed-on-create: tạo quán → seed sẵn demo từ TS seed bundle (**user+menu+sơ đồ bàn/decor**). Nút "Clear demo data" admin-only, block nếu còn open orders, sau đó tombstone demo data, chừa 1 admin. Không local demo mode |
...
## Payment/order
Order draft chỉ lưu DB khi bấm **In/Gửi đơn** qua `submit_order_changes`; bàn trống chưa tạo DB, bàn có order nếu không có draft thay đổi thì nút chính là **Thanh toán**. Submit order open dùng **replace snapshot** item/options. Toàn bộ item về 0 + In/Gửi → order open `void`, table empty. Order/payment dùng `orders.lock_version` + `expectedVersion`; conflict trả `ORDER_VERSION_CONFLICT` và UI refetch. Payment có bảng `payments`; UI MVP chỉ cash, enum chừa `bank_transfer/qr/other`; Complete trước rồi mới render/in final bill, paid order không void trong MVP. Tiền integer VND, item/options snapshot tên+giá, daily `order_no` theo `store_settings.timezone`. Print qua `IPrintPort` HTML/template preview. Kitchen queue không làm, chỉ giữ seam `order_items.status`.

## Ports/seed/tasks
Ports theo domain repo (`Auth/Employee/Menu/FloorPlan/Order/Payment/Report/Settings/Seed/Print`), Core/UI dùng camelCase, adapter map snake_case, repo mặc định lọc `deleted_at is null`, lỗi dùng `AppError` gồm `ORDER_VERSION_CONFLICT`. Seed demo: admin PIN `123456`, cashier PIN `111111`, menu medium kiểu cafe Việt, 2 floor areas medium, decor placeholder nếu thiếu ảnh. Multi-agent sau foundation chia 6 streams.
...
## Bước kế
Implementation contract đã có; bước kế tiếp là review contract rồi chia 6 stream coding foundation-first.
