...
24 change đang ở trạng thái **mới có `proposal.md`**, chưa có `specs/`, `design.md`, `tasks.md`. Đây là chủ ý, không phải thiếu sót.

Vì vậy `openspec validate --changes` báo lỗi `Change must have at least one delta` cho 19 change. Đó là trạng thái mong đợi: delta spec chỉ được viết sau khi các câu hỏi trong proposal đã có câu trả lời. 5 change còn lại thuộc nhóm công cụ và quy trình nên đặt `skip_specs: true` và đã pass.
...
Ba change dưới đây mâu thuẫn với các quyết định đang ghi trong `pos-cafe-context.md` và `docs/requirements.md`. Nếu làm thì phải cập nhật lại các tài liệu đó.
