...
- `src/styles.css` is only the Tailwind entrypoint. It should contain only `@tailwind base;`, `@tailwind components;`, and `@tailwind utilities;`.
...
- `@tailwind` directives in `src/styles.css`.
...
If a legacy semantic selector must remain, put it in the Tailwind plugin/config layer rather than `styles.css`:
...
Expected result: no property matches. `src/styles.css` should only contain the three Tailwind directives.
...
Expected result: no `@apply` in `src/styles.css`.
...
Reject the implementation if it adds or expands `src/styles.css`. The purpose of this handoff is Tailwind-first UI polish, not another custom stylesheet.
