...
| Migration | Schema/RLS/RPC quản lý bằng SQL migrations trong `supabase/migrations/*.sql` |
| Seed | Demo seed nằm trong repo dạng TypeScript seed bundle |
...
| `submit_order_changes(...)` | tạo/cập nhật/huỷ order open bằng replace snapshot items/options; nếu dine-in thì cập nhật table status trong cùng transaction |
...
Exact RPC signatures sẽ chốt ở implementation contract; file này chỉ chốt responsibility + transaction boundary.
