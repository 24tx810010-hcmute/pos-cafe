...
- **Primary actions:** chọn phương thức thanh toán tiền mặt, chọn món để trả (`Chọn tất cả` mặc định bật; checkbox dòng + chip `n/tổng` + nút `+` xoay vòng), nhập tiền khách đưa qua input/keypad (tự điền theo phần đang chọn), quick cash, bật/tắt in hóa đơn, complete payment, refresh stale order, close.
...
- **Important states:** loading, insufficient cash (so với phần đang chọn), chưa chọn món nào, payment error, stale/closed order, receipt/print preview only when print checkbox is enabled.
