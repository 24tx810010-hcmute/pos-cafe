...
- **Purpose:** hoàn tất thanh toán tiền mặt cho order mở và render receipt.
- **User:** cashier/admin.
- **Minimum data:** order detail, total, table/order type, cashier, received amount, change/remaining, expected lock version.
- **Primary actions:** nhập tiền khách đưa, quick cash, complete payment, refresh stale order, close.
- **Important states:** loading, insufficient cash, payment error, stale/closed order, receipt/print preview.
...
- **Purpose:** tra cứu đơn đã tạo/thanh toán theo ngày và filter.
- **User:** cashier/admin.
- **Minimum data:** date range, order summaries, selected order detail, table name map.
- **Primary actions:** lọc ngày/status/type/search, chọn order, xem detail.
